...
/**
 * Plugin Name:     Shipping Lalamove for WooCommerce
 * Plugin URI:      https://github.com/mariovalney/shipping-lalamove-for-woocommerce
 * Description:     Shipping via Lalamove
 * Version:         1.0.0
 * License:         GPLv2 or later
 * Author:          Mário Valney
 * Author URI:      https://mariovalney.com
 * Text Domain:     shipping-lalamove-for-woocommerce
 * Domain Path:     /languages
 *
 * WC requires at least: 4.5
 * WC tested up to: 5.9
 *
 * @package         Shipping_Lalamove_For_WooCommerce
 * @since           1.0.0
 *
...
if ( ! class_exists( 'Shipping_Lalamove_For_WooCommerce' ) ) {

    class Shipping_Lalamove_For_WooCommerce {
         ...
         *
         * @since 1.0.0
         * @return Shipping_Lalamove_For_WooCommerce
         */
        ...
        public function on_activation() {
            do_action( 'slmfw_on_core_activation' );
        ...
        private function add_modules() {
            ...
            foreach ( $results as $result ) {
                ...
                $classname = 'SLMFW_Module_' . $classname;
        ...
        public function load_plugin_textdomain() {
            load_plugin_textdomain( 'shipping-lalamove-for-woocommerce', false, basename( dirname( __FILE__ ) ) . '/languages/' );
        }
        ...
        public function run() {
            ...
            define( 'SLMFW_VERSION', '1.0.0' );
            define( 'SLMFW_PLUGIN_FILE', __FILE__ );
            define( 'SLMFW_PLUGIN_BASENAME', plugin_basename( __FILE__ ) );
            define( 'SLMFW_PLUGIN_PATH', WP_PLUGIN_DIR . '/' . dirname( SLMFW_PLUGIN_BASENAME ) );
            define( 'SLMFW_PLUGIN_DIR', dirname( SLMFW_PLUGIN_BASENAME ) );
            define( 'SLMFW_PLUGIN_URL', plugins_url( '', __FILE__ ) );
            ...
            foreach ( $this->modules as $module_slug => $module ) {
                ...
                if ( property_exists( $module, 'includes' ) ) {
                    foreach ( (array) $module->includes as $class ) {
                        $file = SLMFW_PLUGIN_PATH . '/modules/' . $module_slug . '/includes/' . $class . '.php';
                        if ( file_exists( $file ) ) {
                            require_once $file;
                        }
                    }
 ...
 */
$core = Shipping_Lalamove_For_WooCommerce::instance();
add_action( 'plugins_loaded', array( $core, 'run' ), 99 );

/**
 * A global function like WC()
 */
function Shipping_Lalamove_For_WooCommerce() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
    $modules = Shipping_Lalamove_For_WooCommerce::instance()->get_modules();
    if ( empty( $modules ) ) {
